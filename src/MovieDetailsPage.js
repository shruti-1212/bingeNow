...
const MovieDetailsPage = () => {
  ...
  useEffect(() => {
    fetch(`https://bingenow.onrender.com/movies/${id}`)
  ...
  return (
    <div className="movie-details-page" style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <div className="header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #ccc', paddingBottom: '10px', marginBottom: '20px' }}>
        <h1 style={{ fontSize: '24px', color: '#72A0C1' }}>{movie.title}</h1>
      </div>
      <p style={{color:"#989898"}} > 2025 Movie </p>
      ...
      <div className="content" style={{ display: 'flex', gap: '20px' }}>
        <div className="details" style={{ flex: '2' }}>
          <p style={{ fontSize: '16px', color: '#989898',marginTop:"40px" }}>{movie.synopsis}</p>
          <div className="pricing" style={{ marginTop: '50px',display: 'flex', alignItems: 'center',gap:"60px"}}>
            <button style={{ fontSize: '18px', padding: '10px 40px', marginRight: '10px', backgroundColor: '#72A0C1', border: 'none', borderRadius: '5px', cursor: 'pointer'}}>Rent Price: ${movie.rentPrice}</button>
            <button style={{ fontSize: '18px', padding: '10px 30px', backgroundColor: '#72A0C1', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>Purchase Price: ${movie.purchasePrice}</button>
          </div>
        </div>
